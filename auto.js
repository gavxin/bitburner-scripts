...
export async function main(ns) {
    ...
    const sequences = [
      {
        ...
        action: async () => await nuke_and_exact_hack(),
      },
      {
        name: "money >= 200k$",
      ...
      {
        name: "money >= 500k$",
        condition: async () => ns.getPlayer().money >= 500 * 1000,
        check: async () => ns.fileExists("BruteSSH.exe"),
        action: async () => {
          ns.tprint("\n\nPlease purchas BruteSSH.exe manually!\n");
          ns.tprint("\n\nconnect darkweb\nbuy BruteSSH.exe\n");
          return false;
        },
      },
      {
        name: "money >= 1.5m$",
        condition: async () => ns.getPlayer().money >= 1.5 * 1000 * 1000,
        check: async () => ns.fileExists("FTPCrack.exe"),
        action: async () => {
          ns.tprint("\n\nPlease purchas FTPCrack.exe manually!\n");
          ns.tprint("\n\nconnect darkweb\nbuy FTPCrack.exe\n");
          return false;
        },
      },
  ...
  function b_price() {
    ...
    while (ns.getPurchasedServerCost(ram) < ns.getPlayer().money) {
      s += ns.nFormat(ram * 1024 * 1024 * 1024, '0ib').padStart(6) + " | " + ram.toString().padStart(8) + " | " + ns.nFormat(ns.getPurchasedServerCost(ram), '0.00a$') + "\n";
      ram *= 2;
    }
    ns.tprint(s);
  }
  ...
  function b_list() {
    ...
    ns.getPurchasedServers().forEach(n => {
      s += n.padEnd(12) + " | " + ns.nFormat(ns.getServerMaxRam(n) * 1024 * 1024 * 1024, "0ib").padStart(8) + "\n";
    });
    ns.tprint(s);
  }

  async function b_buy(ram, name) {
    name = name || "s" + ns.nFormat(ram * 1024 * 1024 * 1024, '0ib');
  ...
  async function buy_super_weaken_grow_hack(target, ram = 1048576) {
    let s = await b_buy(ram, `s${ns.nFormat(ram * 1024 * 1024 * 1024, "0ib")}_${target}`);
    if (!s) {
  ...
  async function super_run(agents, target, w, g, h, unit_count = 1) {
    ...
    if (unit_count <= 0) {
      ns.tprint(`unit count too small!`);
      return false;
    }
  ...
  async function super_hack(agents, target) {
    ...
    while (steal_percent > 0) {
      ...
      const unit_count = get_super_run_unit_count(agents, target, weaken_threads, grow_threads, hack_threads);
      ns.tprint(`w:${weaken_threads}, g:${grow_threads}, h:${hack_threads}, unit count:${unit_count}`);
      if (unit_count <= 4) {
        steal_percent -= 1;
        ns.tprint(`${target} unit count too small, steal percent to ${steal_percent}%`);
        continue;
      }
      await super_run(agents, target, weaken_threads, grow_threads, hack_threads);
      ns.tprint(`super hack ${target} started!`);
      return;
    }
